$(document).ready(function () {
    ...
    function t(e) {
        function n() {
            var e = "now" in window.performance ? performance.now() : (new Date).getTime(),
                c = Math.min(1, (e - r) / t),
                l = a[o](c);
            window.scroll(0, Math.ceil(l * (d - u) + u)), window.pageYOffset !== d ? requestAnimationFrame(n) : i && i()
        }
    ...
    window.onresize = e, window.onscroll = function () {
        var t = e(),
            o = n();
        document.getElementById("scrollTop").style.display = o > t ? "block" : "none", o >= 105 ? $(".navbar").addClass("scrolled-nav") : $(".navbar").removeClass("scrolled-nav")
    }, $("#selectMenu").change(function (e) {
        var n = $("#selectMenu option:selected").text();
        if ("Піца" === n) {
            var i = document.getElementById("pizza");
            t(i, o(i), "linear", function () {})
        } else if ("Роли" === n) {
            var a = document.getElementById("roll");
            t(a, o(a), "linear", function () {})
        } else if ("Салати" === n) {
        ...
        var n = o(this);
        t(document.getElementsByClassName("targetTop")[0], n, "linear", function () {})
